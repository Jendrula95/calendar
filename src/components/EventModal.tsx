...
type Label = (typeof labelsClasses)[number];
...
export default function EventModal() {
	...
	const [selectedLabel, setSelectedLabel] = useState<Label>(
		selectedEvent
			? (labelsClasses.find((lbl) => lbl === selectedEvent.label) as Label)
			: labelsClasses[0]
	);
			...
			<form className="bg-white rounded-lg shadow-2xl w-1/4">
					...
					<div>
						{selectedEvent && (
							<span
								...
								className="material-icons-outlined text-red-400 cursor-pointer"
							>
								delete
							</span>
						)}
				...
				<div className="p-3">
					<div className="grid grid-cols-1/5 items-end gap-y-7">
						...
						<p>{daySelected?.format("dddd, MMMM DD") ?? "No date selected"}</p>
						<span className="material-icons-outlined text-gray-400">
							segment
						</span>
						<input
